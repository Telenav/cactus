...
sub build
{
    ...
    {
        die "Publish failed" if !run("cd $folder && mvn -P attach-jars -P sign-artifacts --no-transfer-progress --batch-mode -Dgpg.passphrase='$passphrase' clean install deploy");
    }
    else
    {
